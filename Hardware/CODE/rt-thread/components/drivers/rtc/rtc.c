...
int sntp_set_rtc_time(uint32_t sntp_time)
{
	if(sntp_time==0)
	{
		return -1;
	...
	struct tm new_time;
	sntp_time+=8*60*60;
	p_time=localtime(&sntp_time);
	rt_memcpy(&new_time,p_time,sizeof(struct tm));
	set_date((new_time.tm_year+1900),(new_time.tm_mon+1),new_time.tm_mday);
	set_time(new_time.tm_hour,new_time.tm_min,new_time.tm_sec);
	return 0;
}
#ifdef RTC_SYNC_USING_NTP
...
//    extern time_t ntp_sync_to_rtc(const char *host_name);
	extern void sntp_sync_time(void);
