...
#define JPEG_BUF_SIZE   120 * 1024
#define JPEG_LINE_SIZE  2 * 1024

#define RESET_PIN  GET_PIN(A, 3)
#define PWDN_PIN  GET_PIN(A, 15)

rt_uint32_t *jpeg_data_buf = RT_NULL;
rt_uint32_t JPEG_LINE0_BUF[JPEG_LINE_SIZE * 2];
rt_uint32_t JPEG_LINE1_BUF[JPEG_LINE_SIZE * 2];
...
int rt_ov2640_init(void)
{
	...
	rt_exit_critical();
	#else
	ov_data_rgb565_buf_0=rt_malloc(OV2640_WIDTH*2);
	ov_data_rgb565_buf_1=rt_malloc(OV2640_WIDTH*2);
	#endif
	ov2640_set_image_window_size(0, 0, 800, 600);
    ov2640_set_image_out_size(OV2640_WIDTH, OV2640_HEIGHT);
