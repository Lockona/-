...
#error "Please define RT_USING_MEMHEAP_AS_HEAP"
#endif
...
//    memset(_lcd.front_buf, 0xFF, LCD_BUF_SIZE);
    ...
    else
    {
   	...
   	LTDC_LAYER(&LtdcHandle, 0)->CFBAR &= ~(LTDC_LxCFBAR_CFBADD);
    LTDC_LAYER(&LtdcHandle, 0)->CFBAR = (uint32_t)(_lcd.lcd_info.framebuffer);
	HAL_LTDC_Relaod(&LtdcHandle, LTDC_SRCR_VBR); 
//    MX_DMA2D_Init();
__exit:
    if (result != RT_EOK)
...
//void LCD_Fill_NoDMA(uint32_t x, uint32_t y, uint32_t color)
//{
//    struct drv_lcd_device *lcd;
//    lcd = (struct drv_lcd_device *)rt_device_find("lcd");
//    *(uint32_t *)(lcd->lcd_info.framebuffer + ((LCD_HEIGHT - x) * LCD_WIDTH + y) * (LCD_BITS_PER_PIXEL) / 8) = (color);
//}
//void LCD_Fill(uint32_t start_x, uint32_t start_y, uint32_t end_x, uint32_t end_y, uint32_t color)
//{
//    struct drv_lcd_device *lcd;
//    lcd = (struct drv_lcd_device *)rt_device_find("lcd");
////    LCD_DMA_FillBuf(0, lcd->lcd_info.framebuffer + ((LCD_HEIGHT - (end_x)) * LCD_WIDTH + start_y) * ((LCD_BITS_PER_PIXEL) / 8), end_y - start_y, end_x - start_x, LCD_WIDTH - (end_y - start_y), color);
//}
uint32_t *buf;
#ifndef ART_PI_TouchGFX_LIB
#ifdef DRV_DEBUG
...
int lcd_test()
{
    buf = rt_malloc(20 * 2 * 3);
    memset(buf, 0xff, sizeof((unsigned char *)buf));
    struct drv_lcd_device *lcd;
    lcd = (struct drv_lcd_device *)rt_device_find("lcd");
    while (1)
    {
